...
def load_dataset_nomask(dataname):
    ...
    # Contain both numerical and categorical features
        ...
        # def extend_mask_nomask(mask, bin_num):
        #
    ...
    return train_X, test_X, train_num, test_num, train_cat_idx, test_cat_idx, cat_bin_num
...
def mean_std_nomask(data):
    mean = data.sum(0) / data.shape[0]
    var = ((data - mean) ** 2).sum(0) / data.shape[0]
    std = np.sqrt(var)
    return mean, std
