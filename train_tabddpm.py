...
def main(cfg: CFG):
    ...
    X_train_ord = pre.encodeDf('Ordinal', pre.df_train)  # [nums | cat_indices]
    num_numeric = pre.numerical_indices_np_end
    K = get_K_from_preproc(pre)
    d_in = int(num_numeric + sum(K))

    # normalize numericals only
    mean, std = make_norm_stats(X_train_ord, num_numeric)
    X_train_np = X_train_ord.copy()
    ...
    torch.save(diffusion._denoise_fn.state_dict(), f"{out_dir}/tabddpm_denoiser.pt")
